...
export function validateRequirement(requirement: PhaseRequirement) {
  ...
  if (!requirement.id || typeof requirement.id !== 'string') {
    ...
    warnings.push(
      `Requirement ID "${requirement.id}" doesn't follow standard format (e.g., FR-001, SC-001)`
    )
  }
  ...
  if (!requirement.priority || typeof requirement.priority !== 'string') {
    ...
    warnings.push(
      `Requirement "${requirement.id}" has non-standard priority: ${requirement.priority}`
    )
  }
