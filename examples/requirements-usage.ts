...
import {
  readRequirements,
  readRequirementsFromMultipleFiles,
  MultiFileProcessor,
} from '../lib/requirements-reader'
...
async function exampleUsage() {
  ...
  try {
    ...
    console.log(
      `   Found ${requirements.phaseContext.requirementsCount} requirements in phase ${requirements.phaseContext.phaseId}`
    )
    console.log(
      `   Validation errors: ${requirements.validationResults.filter((v) => !v.isValid).length}`
    )
  ...
  try {
    ...
    if (existingFiles.length > 0) {
      ...
      allRequirements.forEach((result) => {
        console.log(
          `   - ${result.phaseContext.phaseId}: ${result.requirements.length} requirements`
        )
      })
    }
  } catch (error) {
  ...
  try {
    ...
    console.log(
      `   Processed ${result.phaseContext.phaseId} with ${result.requirements.length} requirements`
    )
